...
#################################################
#################################################
#####	Author:	Christon Antony Cardoza		#####
...
def determineUpdates(tagName, oldList, newList, oldConList, newConList, oldFncConList = '', newFncConList = ''):
	...
	if setDiff:
		...
		for i in setDiff:
			if tagName is "Messages":	#msg.xml
				report.write("\n&emsp;id1=\"")
				report.write(i)
	...
	if setDiff:
   	...
   	#msg.xml
		...
		for i in setDiff:
			...
			elif tagName is None:	#table-map
				report.write("\nenvisionName=\"")
				report.write(i)
				report.write("\" \n<div class=\"layer3\">\n<span class=\"expand\"></span><span class=\"green\">new</span>\n<br>")
...
def fieldExtraction(oldXml, newXml, xmlPath):
	...
	if oldChecksum == newChecksum:
		...
		exn_name_list = '<ExtensionKey.*?cefName="(.*?)"'
		exn_con_list = '<(ExtensionKey.*?)/>'
		ven_name_list = '<Vendor2Device.*?product="(.*?)"'
		ven_con_list = '<(Vendor2Device.*?/)>'
#		dvs_name_list = '<device2meta.*?device="(.*?)"'
#		dvs_con_list = '<(device2meta.*?/)>'
		#extract information from old xml
		oldHdrList = re.findall(header_regex, oldXml, flags=re.DOTALL)
		oldMsgList = re.findall(message_regex, oldXml, flags=re.DOTALL)
		oldHdrConList = re.findall(hdrcontent_regex, oldXml, flags=re.DOTALL)
		oldMsgConList = re.findall(msgcontent_regex, oldXml, flags=re.DOTALL)
		...
		oldExnnameList = re.findall(exn_name_list, oldXml, flags=re.DOTALL)
		oldExnconList = re.findall(exn_con_list, oldXml, flags=re.DOTALL)
		oldVennameList = re.findall(ven_name_list, oldXml, flags=re.DOTALL)
		oldVenconList = re.findall(ven_con_list, oldXml, flags=re.DOTALL)
#		oldDvsnameList = re.findall(dvs_name_list, oldXml, flags=re.DOTALL)
#		oldDvsconList = re.findall(dvs_con_list, oldXml, flags=re.DOTALL)
		oldFncCon = re.findall(fnccontent_regex, oldXml, flags=re.DOTALL)
		...
		newMsgList = re.findall(message_regex, newXml, flags=re.DOTALL)
		newHdrConList = re.findall(hdrcontent_regex, newXml, flags=re.DOTALL)
		newMsgConList = re.findall(msgcontent_regex, newXml, flags=re.DOTALL)
		...
		newExnnameList = re.findall(exn_name_list, newXml, flags=re.DOTALL)
		newExnconList = re.findall(exn_con_list, newXml, flags=re.DOTALL)
		newVennameList = re.findall(ven_name_list, newXml, flags=re.DOTALL)
		newVenconList = re.findall(ven_con_list, newXml, flags=re.DOTALL)
#		newDvsnameList = re.findall(dvs_name_list, newXml, flags=re.DOTALL)
#		newDvsconList = re.findall(dvs_con_list, newXml, flags=re.DOTALL)
		newFncCon = re.findall(fnccontent_regex, newXml, flags=re.DOTALL)
		...
		noExtensionKeys = determineUpdates("ExtensionKeys",oldExnnameList,newExnnameList,oldExnconList,newExnconList)
		noVendor = determineUpdates("VendorProducts",oldVennameList,newVennameList,oldVenconList,newVenconList)
#		noDevicetoMeta = determineUpdates("device2meta",oldDvsnameList,newDvsnameList,oldDvsconList,newDvsconList)
	...
	with open(outputName, "a") as report:
		if oldVersion.endswith(".xml") or newVersion.endswith(".xml"):	#xml inputs detected
			...
			if path.endswith(".envision"):
				...
				for root, dirs, files in os.walk(path):	#determine all the components within the folder
					for i in files:
						if i.endswith(".xml"):	#looping all the file within each directory
